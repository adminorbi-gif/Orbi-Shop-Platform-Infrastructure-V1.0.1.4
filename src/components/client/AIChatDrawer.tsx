...
import { X, Send, Bot, User, Sparkles, Camera, Image as ImageIcon, Zap, History, Clock } from "lucide-react";
        ...
        >
          ...
          <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/30 scroll-smooth">
            ...
            {aiChatHistory.map((msg, idx) => (
              <div key={idx} className={`flex items-end gap-2 ${msg.role !== "user" ? "flex-row-reverse justify-start" : "justify-start"}`}>
                ...
                <div className={`max-w-[80%] rounded-2xl p-3 shadow-sm ${msg.role === "user" ? "bg-amber-500 text-white rounded-bl-sm" : "bg-white border border-slate-100 text-slate-800 rounded-br-sm"}`}>
                  {msg.image && (
