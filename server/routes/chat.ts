        ...
        if (securityStatus.flagged) {
            const warningContent = userLang === "sw"
                ? (msg.senderRole === "seller"
                    ? `⚠️ ONYO LA USALAMA (${securityStatus.newFlags}/3): Kushiriki mawasiliano binafsi (physical contacts) na kufanya malipo ya nje ya mfumo (off-platform payments) ni ukiukaji mkubwa wa usalama. Kuuza moja kwa moja kupitia njia binafsi kunaepuka ulinzi wa dhamana (escrow) wa Orbi PaySafe, jambo ambalo linaweza kusababisha upotevu wa fedha na ukiukaji wa usalama wa mteja. Akaunti yako itafungiwa baada ya ukiukaji mara 3.`
                    : `⚠️ ONYO LA USALAMA (${securityStatus.newFlags}/5): Kushiriki mawasiliano binafsi (physical contacts) na kufanya malipo ya nje ya mfumo (off-platform payments) ni ukiukaji mkubwa wa usalama. Kulipa moja kwa moja kupitia namba binafsi kunaepuka ulinzi wa dhamana (escrow) wa Orbi PaySafe, kumaanisha kwamba hatuwezi kukulinda au kukurudishia fedha ikiwa utatapeliwa. Akaunti yako itafungiwa baada ya ukiukaji mara 5.`)
                : (msg.senderRole === "seller" 
                    ? `⚠️ SECURITY WARNING (${securityStatus.newFlags}/3): Sharing physical contacts, personal phone numbers, or arranging off-platform payments is a severe security policy violation. Selling directly via private channels bypasses Orbi PaySafe escrow protection, which can lead to loss of funds. Your account will be banned after 3 violations.`
                    : `⚠️ SECURITY WARNING (${securityStatus.newFlags}/5): Sharing physical contacts, personal phone numbers, or arranging off-platform payments is a severe security policy violation. Paying directly via private numbers bypasses Orbi PaySafe escrow protection, meaning we cannot protect you or refund you if you are scammed. Your account will be banned after 5 violations.`);
